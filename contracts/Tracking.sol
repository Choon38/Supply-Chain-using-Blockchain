...
contract Tracking {
  ...
  mapping(address => Shipment[]) public shipment;
  ...
  _receiver, uint256 _pickupTime, uint256 _distance, uint256 _price) public payable {
  ...
  public{
    Shipment storage shipment = shipments[_sender][_index];
    TyepShipment storage typeShipment = tyepShipments[_index];

    require(shipment.reciver == _receiver, "Invalid reciver.");
  ...
  function getShipment(address _sender, uint256 _index) public view returns (address, address, uint256, uint256, uint256, uint256, ShipmentStatus, bool){
    Shipment memory shipment = shipment[_sender][_index];
    return (shipment.sender, shipment.reciver, shipment.pickupTime, shipment.deliveryTime, shipment.price, shipment.status, shipment.isPaid);

  }

  function getShipment(address _sender) public view returns (uint256) {
